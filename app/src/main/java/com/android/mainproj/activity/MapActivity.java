		...
		extends AppCompatActivity
{
	...
	private ImageButton btn_map_back;
	...
	{
		btn_map_back.setOnClickListener(listener_back_click);
	...
	private final View.OnClickListener listener_back_click = new View.OnClickListener()
	{
		@Override
	...
	private final View.OnClickListener listener_back_detect_pos = new View.OnClickListener()
	{
		...
		public void onClick(View v)
		{
			// 백그라운드로 위치 정보를 호출할 경우 위치 권한이 항상 허용으로 설정되어 있어야 한다.
			// 항상 허용 권한을 설정하기 위해서는 ACCESS_BACKGROUND_LOCATION 권한이 있어야한다.
			// 또한 ACCESS_BACKGROUND_LOCATION 은 권한 요청을 할 경우에 항상 명시적 권한을 거부한 상태로 처리한다.
			// 권한을 허용하기 위해서는 사용자가 직접 설정에서 변경해 주어야 한다.
			...
			{
				// 항상 허용으로 설정되어있을 경우 shouldShowRequestPermissionRationale 의 결과가 false로 반환됨
			}
		}
	};
	...
	private void requestUserPermission(String perm)
	{
		...
		dialog.setDialogOnClickListener(new PermissionDialog.OnDialogClickListener()
		{
			...
			{
				Intent appDetail = new Intent(Settings.ACTION_APPLICATION_DETAILS_SETTINGS, Uri
